...
import { createPeerConnection, getUserMedia, createOffer, createAnswer, isCallingSupported, createSessionDescription } from '../utils/webrtc';
...
export default function VoiceChatScreen() {
  ...
  const startCall = async (friend) => {
    ...
    try {
      const stream = await getUserMedia(true, false);
  ...
  const listenForAnswer = (pc, callId) => {
  ...
  return (
    <View style={styles.container}>
      {incomingCall && (
        <View style={styles.incomingBanner}>
          <View style={styles.incomingInfo}>
            <Icon name="phone-ring" size={24} color="#fff" />
            <Text style={styles.incomingText}>{incomingCall.callerName} is calling...</Text>
          </View>
      ...
      <Text style={[styles.sectionTitle, { marginTop: 20 }]}>All Friends</Text>
      <FlatList
        data={friends}
        keyExtractor={item => item.uid}
        ...
        renderItem={({ item }) => (
          <View style={styles.friendItem}>
            ...
            <TouchableOpacity style={[styles.callBtn, { opacity: item.status === 'online' ? 1 : 0.5 }]} onPress={() => startCall(item)} disabled={item.status !== 'online'}>
              <Icon name="phone" size={20} color="#fff" />
            </TouchableOpacity>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  statusDot: { width: 12, height: 12, borderRadius: 6, position: 'absolute', bottom: -1, right: -1, borderWidth: 2, borderColor: '#2C2C2C' },
  friendInfo: { flex: 1 },
  friendName: { color: '#fff', fontSize: 15, fontWeight: '600' },
  friendStatus: { color: '#888', fontSize: 12, marginTop: 2 },
  callBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#4CAF50', justifyContent: 'center', alignItems: 'center' },
  emptyText: { color: '#888', fontSize: 14, textAlign: 'center', marginTop: 20 },
  callContainer: { flex: 1, backgroundColor: '#1A1A1A', justifyContent: 'center', alignItems: 'center' },
  callInfo: { alignItems: 'center', marginBottom: 60 },
  callAvatar: { width: 100, height: 100, borderRadius: 50, backgroundColor: '#2C2C2C', justifyContent: 'center', alignItems: 'center', marginBottom: 20 },
  callName: { color: '#fff', fontSize: 24, fontWeight: '700', marginBottom: 8 },
  callStatus: { color: '#888', fontSize: 14 },
  callControls: { flexDirection: 'row', alignItems: 'center', gap: 30 },
  muteBtn: { width: 56, height: 56, borderRadius: 28, backgroundColor: '#2C2C2C', justifyContent: 'center', alignItems: 'center' },
  endCallBtn: { width: 64, height: 64, borderRadius: 32, backgroundColor: '#F44336', justifyContent: 'center', alignItems: 'center' },
